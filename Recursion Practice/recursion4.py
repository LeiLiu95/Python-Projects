...
def splitArrayHelper(start,nums,g1,g2):
	if(start>=len(nums)):
		return g1==g2
...
def splitOdd10Helper(start,nums,mult,odd):
	if(start>=len(nums)):
...
def split53Helper(start,nums,tot):
	if(start>=len(nums)):
		return tot==0
	if(nums[start]%5==0):
		return split53Helper(start+1,nums,tot+nums[start])
	else:
		if(nums[start]%3==0):
			return split53Helper(start+1,nums,tot-nums[start])
		else:
			if(split53Helper(start+1,nums,tot+nums[start])):
				return True
			if(split53Helper(start+1,nums,tot-nums[start])):
				return True
	return False
