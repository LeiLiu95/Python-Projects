...
#  Description: Recursion coding bat
...
def changeXY(str):
	...
	if(len(str)==1):
		if(str=='x'):
			...
			return str
	c=changeXY(str[0])+changeXY(str[1:])
	return c
...
def stringClean(str):
	if(len(str)<2):
		return str
	if(str[0]==str[1]):
		return stringClean(str[0]+str[2:])
...
def strCount(str, sub):
	if(str.find(sub)==-1):
		...
		return 1 + strCount(str[str.find(sub)+len(sub):],sub)
...
def strCopies(str, sub, n):
	...
	if(len(str)<len(sub)):
		return False
	if(str.find(sub)!=-1):
		return strCopies(str[str.find(sub)+1:],sub,n-1)
...
def strDist(str, sub):
	if(str.find(sub)==-1):
		...
		return str.rfind(sub) -str.find(sub) +len(sub)
